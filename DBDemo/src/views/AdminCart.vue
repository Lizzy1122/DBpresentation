...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        addToCartTime: [
          { required: true, message: "请选择时间和日期", trigger: "blur" },
        ],
      },
  ...
  methods: {
    seleteAll() {
      this.axios
        .get("http://localhost:3312/cart/selectAll")
        .then((res) => {
          console.log(res);
    ...
    load(pageNum) {
      ...
      this.axios
        .get("http://localhost:3312/cart/selectByPage", {
          params: {
      ...
      this.$refs.formRef.validate((valid) => {
        if (valid) {
          if (this.form.cartID) {
            ...
            this.axios
              .put("http://localhost:3312/cart/update", {
            ...
            this.axios
              .post("http://localhost:3312/cart/add", {
                addToCartQuantity: this.form.addToCartQuantity,
                addToCartTime: this.form.addToCartTime,
                userID: this.form.userID,
                productID: this.form.productID,
    ...
    delBatch() {
      ...
      this.$confirm("您确认批量删除这些数据吗？", "确认删除", {
        ...
        .then((resp) => {
          this.axios
            .delete("http://localhost:3312/cart/delete/batch/", {
              data: this.ids,
            })
            .then((res) => {
              if (res.data.code === "200") {
    ...
    handleDelete(id) {
      this.$confirm("您确认删除吗？", "确认删除", { type: "warning" })
        .then((resp) => {
          this.axios
            .delete("http://localhost:3312/cart/delete/" + id)
            .then((res) => {
              if (res.data.code === "200") {
                this.$message.success("删除成功");
                this.seleteAll();
      ...
      <el-table-column label="操作" key="slot">
        <template #default="scope">
          ...
          <el-button
            size="small"
            type="danger"
            @click="handleDelete(scope.row.cartID)"
            >删除</el-button
          >
        </template>
      </el-table-column>
