...
export default {
  ...
  methods: {
    seleteAll() {
      this.axios
        .get("http://localhost:3312/purchase/selectAll")
        .then((res) => {
          console.log(res);
    ...
    load(pageNum) {
      ...
      this.axios
        .get("http://localhost:3312/purchase/selectByPage", {
          params: {
      ...
      this.$refs.formRef.validate((valid) => {
        if (valid) {
          if (this.form.orderID) {
            ...
            this.axios
              .put("http://localhost:3312/purchase/update", {
    ...
    delBatch() {
      ...
      this.$confirm("您确认批量删除这些数据吗？", "确认删除", {
        ...
        .then((resp) => {
          this.axios
            .delete("http://localhost:3312/purchase/delete/batch/", {
              data: this.ids,
            })
            .then((res) => {
              if (res.data.code === "200") {
    ...
    handleDelete(id) {
      this.$confirm("您确认删除吗？", "确认删除", { type: "warning" })
        .then((resp) => {
          this.axios
            .delete("http://localhost:3312/purchase/delete/" + id)
            .then((res) => {
              if (res.data.code === "200") {
                this.$message.success("删除成功");
                this.seleteAll();
...
<template>
  <div class="container">
    <div style="display: flex; flex-direction: column">
      ...
      <div class="upperdiv" style="display: flex; margin-top: 5px">
        <el-input
          v-model="OrderID"
    ...
    <div class="middlediv" style="display: flex; align-items: center">
      <el-table
        :data="tableData"
        style="width: 100%"
        stripe
