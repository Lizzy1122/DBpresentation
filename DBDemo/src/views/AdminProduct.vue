...
export default {
  ...
  methods: {
    handleAddToCart(id) {
      ...
      this.axios
        .post("http://localhost:3312/product/AddProductToCart", {
    ...
    load(pageNum) {
      ...
      this.axios
        .get("http://localhost:3312/product/selectByPage", {
          params: {
      ...
      this.$refs.formRef.validate((valid) => {
        if (valid) {
          if (this.form.productID) {
            ...
            this.axios
              .put("http://localhost:3312/product/update", {
            ...
            this.axios
              .post("http://localhost:3312/product/add", {
                productName: this.form.productName,
                description: this.form.description,
                price: this.form.price,
                stockQuantity: this.form.stockQuantity,
    ...
    handleDelete(id) {
      this.$confirm("您确认删除吗？", "确认删除", { type: "warning" })
        .then((resp) => {
          this.axios
            .delete("http://localhost:3312/product/delete/" + id)
            .then((res) => {
              if (res.data.code === "200") {
    ...
    delBatch() {
      ...
      this.$confirm("您确认批量删除这些数据吗？", "确认删除", {
        ...
        .then((resp) => {
          this.axios
            .delete("http://localhost:3312/product/delete/batch/", {
              data: this.ids,
            })
            .then((res) => {
              if (res.data.code === "200") {
